...
function showBanknotes() {
    ...
    for (let i = 0; i < banknotes.length; i++) {
        ...
        if (num >= n && inventory[n]) {
            document.querySelector("#output1").innerHTML += `<img src="img (${n}).png">`;
    ...
    if (result < initialAmount) {;
        ...
        document.querySelector("#output1").innerHTML += `<p>סכום שחסר: ${Math.ceil((initialAmount - result) * 100) / 100}</p>`;
    }
}
